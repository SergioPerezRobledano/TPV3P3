...
void Networking::handle_player_state(const PlayerStateMsg &m) {
	...
	if (m._client_id != clientId_) {
		Game::instance()->get_little_wolf().update_player_state(m._client_id, m.x,
				m.y, m.rot);
	}
}
...
void Networking::send_my_info(float x, float y, float rot,
		Uint8 state) {
	PlayerInfoMsg m;
...
void Networking::handle_restart() {
	Game::instance()->get_little_wolf().restartGame();
