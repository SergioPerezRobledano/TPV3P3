...
LittleWolf::LittleWolf(uint16_t xres, uint16_t yres, SDL_Window *window,
		...
		player_id_(0),		
		restart_game_(false),
		elapsed_time_(0){ // we start with player 0
...
void LittleWolf::update() {
	...
	if (!restart_game_ && p.state == ALIVE) {
		spin(p);  // handle spinning
		move(p);  // handle moving
		shoot(p); // handle shooting
	}
	
	if (restart_game_ && sdlutils().currRealTime() > elapsed_time_+5000.0f) {
...
void LittleWolf::send_my_info()
{
	...
	Game::instance()->get_networking().send_my_info(p.where.x,p.where.y, p.theta, p.state);
}
...
void LittleWolf::killPlayer(std::uint8_t id)
{
	...
	for (auto i = 0u; i < max_player; i++) {
		if (players_[i].state == ALIVE)numDead++;
	}

	if (numDead < 2) {
		restart_game_ = true;
		elapsed_time_ = sdlutils().currRealTime();
	}
...
bool LittleWolf::addPlayer(std::uint8_t id) {
	...
	uint16_t orow = /*rand.nextInt(0, map_.walling_height);*/0;
	uint16_t ocol = /*rand.nextInt(0, map_.walling_width);*/0;
...
void LittleWolf::render() {
	...
	if(restart_game_)render_restart_text();
}
...
void LittleWolf::render_restart_text()
{
	std::string msg = ("The game will restart in 5 seconds");
...
void LittleWolf::restartGame()
{
	for (auto i = 0u; i < max_player; i++) {
		...
		// not that player <id> is stored in the map as player_to_tile(id) -- which is id+10
		update_player_info(p.id, p.where.x, p.where.y, p.theta, p.state);
	}
}
